...
{
    public class DeleteUsersConsumer : BackgroundService
    {
        ...
        protected override Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            consumer.Received += async (model, ea) =>
            {
                ...
                try
                {
                    ...
                    var result = await DeleteUsers(message);
        ...
        private async Task<ResultModel> DeleteUsers(string message)
        {
            var messageDTO = JsonConvert.DeserializeObject<List<string>>(message);
            using (var scope = _scopeFactory.CreateScope())
            {
                IUserService _userService = scope.ServiceProvider.GetRequiredService<IUserService>();
                var result = await _userService.Delete(messageDTO);
                return result;
            }
        }
    }
